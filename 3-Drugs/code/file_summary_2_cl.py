...
# this is the command line version of file_summary_2.py
import argparse
...
###################################################
# filter filenames by a prefix match.
...
############################################################################
#
# support for comamnd line execution
...
def mainfunction2():
    parser = argparse.ArgumentParser(description='Code for extracting performance tables from logs of drug enrichemnt code output')

    parser.add_argument('--mode',
                        action='store',
                        choices = ['createdirectory', 'analyzedirectory'],
                        default = 'analyzedirectory',
                        help = "Switch to crete or analyze directories for 4 diseases (astham, arthritis, colorectal cancer, preostate caner) and three covid19 data sets (pbnc, balf, cells)")


    parser.add_argument('--prefix',
                        action='store',
                        choices = ['merge', 'test', 'global'],
                        default = 'test',
                        help = "Prefix of log files to parse. test is the defeult for single algorithm runs, merge for merge runs on single data, global for merge runs on multiple data")

    parser.add_argument('-dir',
                        dest ='directory',
                        action='store',
                        help = "Relative name of directory to analyze")

    parser.add_argument('-out',
                        dest ='outfilename',
                        action='store',
                        help = "Name of output file, including the .csv suffix")
    ...
    print('Outfile: ', args.outfilename)

    if args.mode == 'createdirectory' :
        generatedirectorystucture(namesdatasets, namesoptions)
        return None
    ...
    if args.outfilename  == None :
        ...
        print "Out file name not specified"

    elif args.directory != None :
        
        if testexistencedirectory(args.directory):

# example rundataextraction('./modulation-colorectal-cancer-results-pval-nofda-allposonly', 'merge', sel , 'merge-crc-data-nofda-allpos-pval-2021-04-10.csv', True, ';')            
            rundataextraction(args.directory, args.prefix, sel , args.outfilename, True, ';')
